...
async function getPortfolioItems(): Promise<PortfolioItem[]> {
  ...
  return data.map((item: any) => ({
    ...
    media: {
      type: item.acf.media_type,
      src: item.acf.media_type === 'video' ? item.acf.video_url : (item.acf.image || ''),
    },
      ...
      >
        <div className="aspect-[4/3] overflow-hidden rounded-2xl relative">
          {item.media.type === "video" ? (
            ...
            <Image
              src={imageError || !item.image ? '/default-portfolio-image.jpg' : item.image}
              alt={item.title}
              width={800}
              height={600}
              className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105"
